...
public class IkHelper : MonoBehaviour
{
    ...
    {
        //_animator.GetIKPosition(foot) 获取到的是初始值（也就是动画在未IK前的状态）;
        Vector3 targetIkPosition = _animator.GetIKPosition(foot);
        //Debug.Log($"当前帧 {Time.frameCount} GetIKPosition {foot} IKPosition {targetIkPosition}");    
        ...
        if (positionIkHolder != Vector3.zero)
        {
            ...
            //由于_animator.GetIKPosition(foot) 获取到的是初始值（也就是动画在未IK前的状态），并不是累加值，所以此处的yVar才是真正的累加值，而不是实际值所以 所以可以 +=;
        ...
        //Debug.Log($"当前帧 {Time.frameCount} SetIKPosition {foot} IKPosition {targetIkPosition}");
